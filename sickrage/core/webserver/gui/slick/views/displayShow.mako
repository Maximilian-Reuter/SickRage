...
<%!
    ...
    from sickrage.core.common import SKIPPED, WANTED, UNAIRED, ARCHIVED, IGNORED, FAILED, DOWNLOADED, WATCHED
        ...
        <tr class="${Overview.overviewStrings[epCats[epStr]]} season-${curSeason} seasonstyle"
            ...
            <td class="col-name">
                % if epResult[b"description"] != "" and epResult[b"description"] is not None:
                    ...
                    % endif
                % if epResult[b"location"] != "":
               
                    <a class="epDisplay" href="displayEpisode?show=${show.indexerid}&amp;season=${epResult[b"season"]}&amp;episode=${epResult[b"episode"]}">${epResult[b"name"]}</a>
                % else:
                    ${epResult[b"name"]}
                % endif
            </td>
